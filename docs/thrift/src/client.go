...
func handleClient(client *sample.CalculatorClient) (err error) {
	...
	work := sample.NewWork()
	work.Op = sample.Operation_DIVIDE
	work.Num1 = 1
	work.Num2 = 0
	quotient, err := client.Calculate(defaultCtx, 1, work)
	if err != nil {
		switch v := err.(type) {
		case *sample.InvalidOperation:
	...
	work.Op = sample.Operation_SUBTRACT
	work.Num1 = 15
	work.Num2 = 10
...
func runClient(transportFactory thrift.TTransportFactory, protocolFactory thrift.TProtocolFactory, addr string, secure bool, cfg *thrift.TConfiguration) error {
	...
	return handleClient(sample.NewCalculatorClient(thrift.NewTStandardClient(iprot, oprot)))
}
